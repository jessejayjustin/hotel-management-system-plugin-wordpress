...
<section class="page-section bg-dark">
		...
		<div class="container">	
				<div class="col-lg-12">	
						...
						$qry = $wpdb->get_results("SELECT distinct(category_id),category_id from wp_rooms where id not in (SELECT room_id from wp_checked where '$date_in' BETWEEN date(date_in) and date(date_out) and '$date_out' BETWEEN date(date_in) and date(date_out))", ARRAY_A);
						foreach ($qry as $row) {
						?>
						<div class="card item-rooms mb-3">
							<div class="card-body">
								<div class="row">
								<div class="col-md-5">
									<img src='http://localhost/wordpress/wp-content/plugins/hotel-booking/assets/img/<?php echo $cat_arr[$row['category_id']]['cover_img'] ?>' alt=""> 
								</div>
...
<script>
  $j('.book_now').click(function(){
    uni_modal('Book','http://localhost/wordpress/wp-content/plugins/hotel-booking/admin/book.php?in=<?php echo $date_in ?>&out=<?php echo $date_out ?>&cid='+$j(this).attr('data-id'));
  });
</script>